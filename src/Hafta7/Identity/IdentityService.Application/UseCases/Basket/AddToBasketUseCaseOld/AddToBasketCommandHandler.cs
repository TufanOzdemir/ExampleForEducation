...
namespace IdentityService.Application.UseCases.Basket.AddToBasketUseCaseOld
{
    public class AddToBasketCommandHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
        public void Execute(AddToBasketCommand request)
        {
            ...
            var basket = IdentityService.Domain.Entities.Basket.Create(currentUserService.UserId, product);
